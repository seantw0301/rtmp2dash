#!/usr/bin/env bash
# Stop the background rtmp2dash process started by start.sh.
set -euo pipefail
...
if [[ ! -f "$PID_FILE" ]]; then
  # Fallback: match our binary + config if pid file is missing
  pids="$(pgrep -f "${ROOT}/target/release/rtmp2dash" || true)"
  if [[ -z "${pids}" ]]; then
